...
function modificar(){
	...
	}else if(esOpcionColgable(filaSel[0])){//Si intenta modificar la operación fuera de menú
		alert("No se puede modificar la operacion fuera de menú.");
	}else {
...
function esOpcionColgable(strCodigoComparacion){
	...
	if (strCodigoComparacion == "OPFMENU" || strCodigoComparacion == "OPFMENUL"){
...
function colgar(){
	...
	if (filaSel != null){
		if (!esOpcionColgable(filaSel[0])){
			alert("Sólo se pueden colgar elementos en 'Operaciones Fuera de Menú'.");
			return;
		}
	}
	// END DEFINA
	var rc = mostrarDialogoOperacionesFueraMenu();
	if (filaSel == null){
...
<table width="100%"  border="0" cellspacing="8" cellpadding="0">
  <tr>
    <td height="22" class="CabeceraRelacionada2">&nbsp;&nbsp;OPERACIONES FUERA DE MENÚ SIN ESCENARIO :: Modificar :: Ver &aacute;rbol </td>
  </tr>
  <tr>
    <td class="TituloTabla">&Aacute;rbol de Operaciones Fuera de Menú Sin Escenario</td>
  </tr>
  <tr>
    <td align="right"><table width="100%" border="0" cellpadding="0" cellspacing="2" class="TablaDatos">
      ...
      <tr class="Pijama<%=((p=!p)?"1":"2")%>">
       ...
       %>
        <td width="97%"  class="TextoTablaN"><img src="images/organigrama.gif" border="0">[<%=(isPesado)?codOperacion:codOperacionLigera%>]  Operaciones Fuera de Menú </td>
