...
function HM_f_StartIt() {
    ...
    if (HM_ClickKill) {
        ...
        document.onmousedown = function(e){var retVal = routeEvent(e);HM_f_PageClick();HM_f_OtherMouseDown();return !(retVal==false)}
    }
    else {
...
function HM_f_MakeMenu(menucount) {
    ...
    NewMenu = window[HM_MenuIDPrefix + menucount];

    if(!NewMenu) {
        NewMenu = window[HM_MenuIDPrefix + menucount] = new Layer(HM_CurrentTree.MenuWidth,window);
        if(HM_ScrollEnabled) {
    ...
    while (HM_CurrentMenu.itemCount < HM_CurrentMenu.maxItems) {
        ...
        HM_CurrentItem = window[HM_ItemIDPrefix + menucount + "_" + HM_CurrentMenu.itemCount];
        if(!HM_CurrentItem) {
            if(HM_StatusDisplayBuild) status = "Creating Hierarchical Menus: " + menucount + " / " + HM_CurrentMenu.itemCount;
            HM_CurrentItem = window[HM_ItemIDPrefix + menucount + "_" + HM_CurrentMenu.itemCount] = new Layer(HM_CurrentTree.ItemWidth - (HM_ItemPadding*2),HM_CurrentMenu.scrollParent);
...
function HM_f_PopDown(menuname){
    ...
    menuname = menuname.replace("elMenu",HM_MenuIDPrefix);
    var MenuToHide = window[menuname];
...
function HM_f_EnableScrolling(topedge){
    ...
    var WindowHeight = window.innerHeight;
    this.clip.height = (Math.max(WindowHeight,30) - 20);
    if(!this.scrollbarsCreated) this.createScrollbars();
    this.checkScroll();
}

function HM_f_CreateScrollbars(){
    var TopScrollElement = new Layer(this.tree.MenuWidth,this);
    with(TopScrollElement){
        top = 0;
        left = 0;
        clip.width = this.tree.MenuWidth;
        clip.height = HM_ScrollBarHeight + HM_BorderWidth;
        visibility = "hide";
        bgColor = this.tree.BorderColor;
    }
    var TopScrollElementLyr = new Layer(this.tree.ItemWidth,TopScrollElement);
    with(TopScrollElementLyr){
        top = HM_BorderWidth;
        left = HM_BorderWidth;
        clip.width = this.tree.ItemWidth;
        clip.height = HM_ScrollBarHeight -  HM_BorderWidth;
        visibility = "inherit";
        bgColor = HM_ScrollBarColor;
        }

    var TopScrollElementImgLyr = new Layer(this.tree.ItemWidth,TopScrollElementLyr);
    with(TopScrollElementImgLyr){
        visibility = "inherit";
        top = (TopScrollElementLyr.clip.height - HM_ScrollImgHeight)/2;
        left = (this.tree.ItemWidth - HM_ScrollImgWidth)/2;
        document.write("<IMG VSPACE=0 HSPACE=0 SRC='" + HM_ScrollImgSrcTop +"'>");
        document.close();
    }

    var BotScrollElement = new Layer(this.tree.MenuWidth,this);
    ...
    var BotScrollElementLyr = new Layer(this.tree.ItemWidth,BotScrollElement);
    ...
    this.scrollbarTop.onmousedown = function(){HM_CurrentMenu.startScroll(true);return false};
    this.scrollbarBot = BotScrollElement;
    this.scrollbarBot.captureEvents(Event.MOUSEDOWN);
    this.scrollbarBot.onmousedown = function(){HM_CurrentMenu.startScroll(false);return false};
...
function HM_f_StartScroll(up){
    ...
    window.onmouseup = HM_f_EndScrollCapture;
}
...
function HM_f_CheckScroll(){
    ...
    if(ScrollEl.topCount <= HeightDiff) {
        ...
        this.scrollbarBot.top = (MenuHeight - HM_ScrollBarHeight - HM_BorderWidth);
